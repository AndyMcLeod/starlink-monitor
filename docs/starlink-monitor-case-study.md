...
| 19 | The dish re-selects its satellite every 15 s, on a fixed grid anchored 12 s past the minute; fold that into the estimate | Feature (domain knowledge) | Estimator phase-locked to the handoff grid, with a live countdown to the next handoff |
